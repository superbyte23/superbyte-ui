(function () {
  try {
    ...
    // Kick off loading of the fonts actually in use before first paint so text
    // never swaps from the fallback (font-display: swap) and shifts the restored
    // scroll position — the active body family plus Ubuntu Mono (badges,
    // breadcrumbs, stat values, code). Cached fonts resolve near-instantly.
    if (document.fonts) {
      var fam = (m.font && FONTS[m.font]) ? FONTS[m.font].name : 'Ubuntu';
      try {
        document.fonts.load('400 14px "' + fam + '"');
        document.fonts.load('500 14px "' + fam + '"');
        document.fonts.load('700 14px "' + fam + '"');
        document.fonts.load('400 14px "Ubuntu Mono"');
        document.fonts.load('700 14px "Ubuntu Mono"');
      } catch (e) {}
    }
